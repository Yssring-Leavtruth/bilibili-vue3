...
if (userInfo.value.isLogin && isLoading.value) {
    historyApi.video(userInfo.value.mid).then(res => {
        res.data.list.forEach(item => {
            if (item.uri) {
                item.link = item.uri
            } else {
                item.link = 'https://www.bilibili.com/video/' + item.history.bvid
            }
...
function getProgress(item: object): void {
    const progress = Math.floor((item.progress / item.duration) * 100)
